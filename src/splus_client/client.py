...
class SPlusClient:
    ...
    - نوشته شده توسط github.com/RezaR2d
    - ارسال پیام و فایل (با قابلیت پاسخ واقعی)
    - دریافت آپدیت‌های real-time
    - فیلتر پیام‌های خود کاربر
    - حذف پیام های کاربر
    - پین و ان پین پیامها در چت
    """
    LOGIN_URL = "https://web.splus.ir/"
    def __init__(self, session_file="my.session", display_logs=True):
        ...
        has_session = os.path.isfile(self.SESSION_FILE)
    ...
    def wait_until_ready(self,driver, max_wait=90):
        ...
        while time.time() - start < max_wait:
            try:
                ...
                if has_floader:
                    self._log("⚠️ بارگذاری طول کشید → رفرش صفحه...")
                    driver.refresh()
                    time.sleep(4)
                    continue
